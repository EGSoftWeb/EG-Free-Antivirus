...
int MainProcessURl20(const std::string& mkey, int& result)
{
	...
	std::string mainurl("http://egsoftweb.in/MachineKey20.aspx?MachineKey=");
	mainurl.append(mkey);
	...
	SuccessAppWinLogW(L"Result From the egsoftweb: %s", Result.wc_str());
...
{
	const int bufLen = 1024;
	std::string mainurl("http://egsoftweb.in/Checking-RegisterKey.aspx?RegisterKey=");
	mainurl.append(rkey);
	...
	SuccessAppWinLogW(L"Result From the egsoftweb: %s", rkeyResult.wc_str());
